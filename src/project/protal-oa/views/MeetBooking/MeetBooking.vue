<template>
  <div class="home page-layout qui-fx-ver">
    <meet-record ref="form" @submit-form="submitForm" title="会议纪要" v-model="editTag" :id="editId">
    </meet-record>
    <sign-record
      title="签到统计"
      type="meeting"
    ...
    <search-form is-reset @search-form="searchForm" :search-label="searchLabel">
      <div slot="left">
        <a-button icon="plus" class="add-btn" @click.stop="addBooking('0')">会议发布</a-button>
      </div>
    </search-form>
    <table-list :page-list="pageList" :columns="columns" :table-list="bookingList">
      <template v-slot:actions="action">
        ...
        <a-tooltip placement="topLeft" title="会议纪要" v-if="action.record.status !== '未使用'">
          <a-button
            size="small"
...
export default {
  ...
  methods: {
    ...
    addBooking(type, record) {
      if (type !== '0') {
        this.$router.push({ path: '/meetBooking/addMeetBooking', query: { id: record.id, type } })
      } else {
        this.$router.push({ path: '/meetBooking/addMeetBooking', query: { type } })
      }
    },
    goDetail(record) {
    },
