  ...
  >
    ...
    >
      <div v-for="(item, index) in formData" :key="index">
        ...
        >
          <a-radio-group buttonStyle="solid" v-decorator="[item.value, {initialValue: item.initValue, rules: [{ required: item.required, message: item.placeholder }]}]">
        ...
        >
          <a-checkbox-group
            v-decorator="[item.value, {initialValue: item.initValue, rules: [{ required: item.required, message: item.placeholder }]}]"
        ...
        >
          <a-select
            v-decorator="[
              item.value,
              { initialValue: item.initValue, rules: [{ required: item.required, message: item.placeholder }]}
            ]"
            :placeholder="item.placeholder"
          >
        ...
        >
          <a-date-picker v-decorator="[item.value, {initialValue: moment(item.initValue || new Date(), 'YYYY-MM-DD'), rules: [{ required: item.required, message: item.placeholder }]}]" />
        </a-form-item>
        <!--日期区间-->
        <a-form-item
          v-bind="formItemLayout"
        ...
        >
          <a-range-picker v-decorator="[item.value, {initialValue: [moment(item.initValue[0] || new Date(), 'YYYY-MM-DD'), moment(item.initValue[1] || new Date(), 'YYYY-MM-DD')], rules: [{ required: item.required, message: item.placeholder }]}]" />
        </a-form-item>
      </div>
    </a-form>
  </a-modal>
