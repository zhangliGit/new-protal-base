...
const projectName = 'cloud-admin' // 此处写项目名作为存储值，避免不同项目冲突
const localData = window.localStorage.getItem(projectName) || JSON.stringify({})
...
const home = {
  ...
  mutations: {
    ...
    updateState(state, { key, data, isLocal = true }) {
      if (isLocal) {
        const localData = JSON.parse(localStorage.getItem(projectName) || JSON.stringify({}))
        localData[key] = data
