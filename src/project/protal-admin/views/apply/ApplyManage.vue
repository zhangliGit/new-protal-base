  ...
  <div class="page-layout qui-fx-ver">
    ...
    <table-list
      ...
      <template v-slot:totalNums="totalNum">
        <div :class="['table-total-num',{'num-cursor': totalNum.record.platform !== 1}]" @click="schoolDetail(totalNum.record)">{{ totalNum.record.platform === 1 ? '平台' : totalNum.record.schoolSum }}</div>
      </template>
      <template v-slot:actions="action">
        <a-tooltip placement="topLeft" title="详情">
          <a-button size="small" class="detail-action-btn" icon="ellipsis" @click.stop="goDetial('/apply/detail',action)"></a-button>
        </a-tooltip>
...
export default {
  ...
  methods: {
    ...mapActions('home', [
      'getApply', 'delApply', 'addApply', 'updateApply', 'getApplySchool'
    ]),
